...
│   ├── main.py                     ← App entry point, router registration, DB migrations
│   │
│   ├── auth/
│   │   ├── models.py               ← User CRUD, bcrypt hashing, auth schema migration
...
│   │
│   ├── core/
│   │   ├── ai_service.py           ← Groq API client, generate_response(), extract_with_groq()
│   │   └── model.py                ← Re-exports generate_response (backward compat)
│   │
│   ├── memory/
│   │   ├── database.py             ← PostgreSQL connection
│   │   ├── schema.py               ← All table creation (users, memories, personal_info, tasks…)
│   │   ├── memory_manager.py       ← Save/fetch memories, chat history, earliest recall
│   │   └── personal_info.py        ← Structured key/value facts store + direct answer builder
│   │
│   ├── retrieval/
│   │   ├── embedder.py             ← Text → 384-dim vectors (all-MiniLM-L6-v2)
│   │   ├── vector_store.py         ← FAISS index per user (disk-persisted, relevance threshold)
│   │   └── rag_engine.py           ← Intent-aware RAG: personal_info / task / chat routing
│   │
│   ├── processing/
...
│   │
│   ├── tasks/
│   │   ├── task_detector.py        ← Strict NLP task detection (no false positives)
│   │   └── task_manager.py         ← Task CRUD + auto-create from chat
│   │
│   ├── utils/
│   │   ├── voice_stt.py            ← Whisper speech-to-text (available for future use)
│   │   ├── voice_tts.py            ← pyttsx3 text-to-speech (available for future use)
│   │   ├── cache.py                ← LRU response cache (200 entries, 10min TTL)
│   │   ├── db_pool.py              ← PostgreSQL connection pool
│   │   └── optimizer.py            ← No-op stub (ONNX removed, Groq API used)
│   │
│   └── api/
...
│       ├── voice.py                ← Voice endpoints (available for future use)
│       ├── optimization.py         ← GET /optimize/cache-stats, model-info
│       └── unique_features.py      ← Memory scoring, mood logging, mood patterns
│
├── soulsync-frontend/              ← React + Tailwind CSS frontend
│   ├── src/
│   │   ├── App.jsx                 ← Main app layout, voice mode wiring, session reset
│   │   ├── main.jsx                ← Router, AuthProvider, key-based session isolation
│   │   ├── index.css               ← Tailwind + custom palette (soul/glow/surface)
│   │   ├── context/
│   │   │   └── AuthContext.jsx     ← JWT auth state, login/signup/logout
│   │   ├── api/
│   │   │   └── soulsync.js         ← Axios client with JWT auto-injection
│   │   ├── pages/
│   │   │   ├── Landing.jsx         ← Full landing page (hero, features, carousel, CTA)
│   │   │   ├── Login.jsx           ← Login form with validation
│   │   │   └── Signup.jsx          ← Signup form with password strength meter
│   │   └── components/
│   │       ├── Header.jsx          ← Logo + status + user + Voice Mode button + logout
│   │       ├── ChatWindow.jsx      ← Message bubbles + typing indicator + suggestion chips
│   │       ├── ChatInput.jsx       ← Text input + inline mic + send
│   │       ├── TaskPanel.jsx       ← Task list + create/complete/delete
│   │       ├── InsightPanel.jsx    ← Emotion bars + suggestions
│   │       └── UseCaseCarousel.jsx ← Interactive use-case carousel (landing page)
│   ├── tailwind.config.js
│   ├── postcss.config.js
│   └── package.json
│
...
│   ├── test_language.py            ← Language detection tests (English/Hindi/Hinglish)
│   ├── test_memory_system.py       ← Memory + intent + recall tests
│   ├── test_task_insight.py        ← Task detection + insight API tests
│   ├── test_recall.py              ← Chronological recall detection tests
│   └── clean_venv.py               ← Remove unused packages from venv
│
├── docker/
│   ├── Dockerfile.backend
│   ├── Dockerfile.frontend
...
```bash
cd C:\Users\techi\Downloads\Surjeet\chatbot
```
...
Edit `soulsync-ai/.env` (already configured, verify values):
...
> ⚠️ **The backend must always run inside `soulsync_env`.** The startup scripts enforce this automatically — they verify the venv Python and all required packages before starting. If you try to run with system Python, packages like `groq`, `psycopg2`, and `jose` will be missing.
...
```
[OK] Venv Python : ...\soulsync_env\Scripts\python.exe
...
```bash
# Terminal 1 — Backend (venv enforced automatically)
powershell -ExecutionPolicy Bypass -File soulsync-ai\start_backend.ps1
...
```bash
# Always use venv
soulsync_env\Scripts\python.exe scripts\<script>.py

# Available scripts
scripts\test_groq_integration.py   # Groq API: basic chat, memory recall, speed
scripts\test_language.py           # Language detection: English, Hindi, Hinglish
scripts\test_memory_system.py      # Intent detection, personal info, task detection
scripts\test_task_insight.py       # Task API + insight API live tests
scripts\test_recall.py             # Chronological recall detection
scripts\seed_users.py              # Seed 5 demo users with full data
scripts\verify_seed.py             # Verify seeded data counts per user
scripts\clean_venv.py              # Remove unused packages from venv
```
...
- All 10 backend modules
- React frontend with Tailwind CSS
- AI model integration and persona
- RAG pipeline with FAISS
- Voice system (STT + TTS)
- ONNX optimization (8.39x speedup)
