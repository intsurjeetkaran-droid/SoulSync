  ...
  return (
    <div className={`flex gap-3 animate-slide-up ${isUser ? "flex-row-reverse" : "flex-row"}`}>
      ...
      <div className={`w-8 h-8 rounded-full flex items-center justify-center
        ...
        {isUser
          ? <User size={14} className="text-surface-950" />
          : <Brain size={14} className="text-soul-400" />}
      </div>
      ...
      <div className={`max-w-[78%] flex flex-col gap-1.5
        ...
        <div className={`w-full rounded-2xl text-sm
          ${isUser
            ? "bg-gradient-to-br from-soul-600 to-soul-700 text-white rounded-tr-sm shadow-md shadow-soul-900/30 px-4 py-3"
            : "bg-surface-800/90 text-surface-100 rounded-tl-sm border border-surface-700/60 px-4 py-3"}`}>
        ...
        <div className="flex items-center gap-2 flex-wrap px-1">
          ...
          {msg.memories?.length > 0 && (
            <span className="inline-flex items-center gap-1 text-[10px]
              ...
              <Zap size={9} />
              {msg.memories.length} memor{msg.memories.length > 1 ? "ies" : "y"} recalled
            </span>
          )}
...
function TypingIndicator() {
  return (
    <div className="flex gap-3 animate-fade-in">
      <div className="w-8 h-8 rounded-full bg-surface-800 border border-surface-700
    ...
    <div className="flex-1 overflow-y-auto px-5 py-6 space-y-5
      ...
      {messages.length === 0 && (
        <div className="flex flex-col items-center justify-center h-full
                        text-center gap-5 py-16 animate-fade-in">
          <div className="relative">
            <div className="w-20 h-20 rounded-3xl bg-gradient-to-br from-soul-600/20 to-soul-800/20
                            border border-soul-600/25 flex items-center justify-center">
              <Brain size={36} className="text-soul-400" />
            </div>
